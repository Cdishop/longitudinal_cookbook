...
The phenomena that organizational researchers study unfold over time: they have been described as "sequences of events that play out within each person's stream of experience" [@beal2015, p. 5] that "are not static but instead develop, change, and evolve over time" [@pitariu2010]. Moreover, organizations are systems with many connected parts, and systems are inherently dynamic. Studying these systems and processes, therefore, requires paying attention to how things happen over time; it puts us in a better position to understand them [@kozlowski2003] and can lead to new and interesting insights. 
...
Armed with repeated observations, there are then different inferences and research questions we can explore to gain a better understanding of how organizational phenomena unfold over time. @hardy2018 propose and find support for growth trends in self-regulation, metacognition, and performance. @jones2016 conclude that concealing behaviors among pregnant women lead to greater subsequent physical health. @johnson2014 argue that "justice is dynamic: The frequency of actors' justice behaviors varies day to day" (p. 10), and these daily fluctuations predict daily changes in depletion. Finally, @meier2013 suggest that the effects of work stressors on counterproductive work behaviors are not substantially different across different time lags.(**Note: I recognize that it might be better to put actual questions in this paragraph instead**) 

Of course, none of these inferences perfectly discovers the data generating mechanism. Rather, each asks an interesting and important question about a DV and how it relates to an IV. Only with lots of asking about lots of different patterns of relationships across the variables could we piece together one (of many) possible representation(s) of the data generating process (hopefully having a good theory to guide the way).

Finally, we find many sophisticated modeling strategies across these studies. @meier2013 present a sequence of path models that test increasingly longer time lags. @hardy2018 and @jones2016 employ bivariate cross-lagged latent growth curves, an approach similar to the latent change model used by @ritter2016. We also find complex hierarchical linear models in various event-sampling studies [e.g., @koopman2016; @rosen2016].

We want to unpack and bring attention to the inferences and models researchers can apply when they study phenomena over time. There are many of them, and in this paper we explain each inference and then connect it to a model so that researchers have a clear application: if you are interested in this inference, here is a model that can be used to explore it. 

Below, we do these things... Contributions: We believe this summary will help any newcomers who want to make inferences with their longitudinal data but are unaware of what types of inferences and models are available. Second, this paper is meant to show seasoned researchers other possibilities -- other inferences and models that may augment how they currently approach longitudinal data. Third, show the similarities and dissimilarites between inferences. Sometimes these studies appear unrelated due to the different langauge used among various content or statistical modeling areas -- but we believe our discussion will reveal some of their covert similarities. Fourth, modeling literature can be sophisticated and tough to parse for researchers with only graduate level training, we help to clarify. Fifth, provide code to estimate models. 



of research questions and inferences available to researchers that are interested in how something unfolds across time,  different inferences, and different models. we want to explain the inferences and then link them to a model. 


Bring attention
link inferences to models
We want to bring attention to this new and exciting literature. Specifically, in this paper we describe some of the common inferences and models researchers employ with longitudinal data. Sometimes these studies appear unrelated due to the different language used among various content or statistical modeling areas -- but we believe our discussion will reveal some of their covert similarities. We discuss the inferences, identify literature examples and hypotheses that are consistent with each, point to various models being used to support them, provide the code necessary to both estimate and understand what the models are doing, and unpack some of the strengths and weaknesses of each approach. Our paper is meant for two audiences. First, 
