...
Levels describe the variable at one moment or averaged across a span of
time. That is, if we put a variable on the \(y\) axis and plot its
values against time on the \(x\) axis, we can explore the value that it
takes at time \(t\), or the value that it takes on average across any
span of \(t\).
...
initially high. What we are doing here is making comparisons between the
level of one variable and the level of another. We are doing so in a
descriptive way. We can also produce a quantitative statement about the
extent to which levels are related.
...
high, and when affect is low is performance also low? Figure
\ref{level_relation} shows this inference graphically. The top panel
plots affect and performance trajectories across time. The colored
...
Figure \ref{trend} demonstrates trend differences, where the red line
shows positive, increasing trend, the blue line shows negative,
decreasing trend, and the green line shows a curvilinear trajectory.
Keep in mind that curvilinear and linear trajectories are both
...
observed variables. For example, we might discover that affect and
performance trends both decrease, but there is greater variability
across units in the affect trend. Or we might learn that affect has a
negative trend while performance has a positive trend.
...
Figure \ref{trend_relation} (no figure yet).
